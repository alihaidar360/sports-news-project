...
export default function Hero({ articles, currentSport }: HeroProps) {
  const list = articles ?? [];
  const main = list[0];
  ...
  const currentHero = sportHeadings[currentSport] || sportHeadings["all"];
      ...
      <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        <div className="lg:col-span-6 fade-in">
          ...
          <div className="mt-8 flex items-center gap-3">
            <Link
              href={`/${main.sport?.slug || "sports"}/${main.slug?.current || main.slug}`}
        ...
        <div className="mt-12 grid sm:grid-cols-3 gap-4">
          {trio.map((a) => {
            ...
            return (
              <Link
                key={a._id}
                href={`/${a.sport?.slug || "sports"}/${a.slug?.current || a.slug}`}
